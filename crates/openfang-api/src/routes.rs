...
struct CodexFlowState {
    ...
    expires_at: Instant,
    /// Whether the flow has been completed (tokens obtained).
    completed: bool,
}

/// Active Codex PKCE flows, keyed by poll_id.
static CODEX_FLOWS: LazyLock<DashMap<String, CodexFlowState>> = LazyLock::new(DashMap::new);

/// Pending authorization codes received via callback, keyed by state parameter.
static CODEX_CALLBACKS: LazyLock<DashMap<String, String>> = LazyLock::new(DashMap::new);

/// POST /api/providers/openai-codex/oauth/start
///
/// Initiates a Codex OAuth PKCE flow. Returns an authorization URL that the
/// user should open in their browser, and a poll_id for checking the flow status.
pub async fn codex_oauth_start() -> impl IntoResponse {
    // Clean up expired flows
    CODEX_FLOWS.retain(|_, state| state.expires_at > Instant::now());
    CODEX_CALLBACKS.retain(|_, _| true); // callbacks are cleaned up when matched

    let auth = openfang_runtime::codex_oauth::build_auth_url();
    ...
    CODEX_FLOWS.insert(
        ...
        CodexFlowState {
            ...
            expires_at: Instant::now() + std::time::Duration::from_secs(300), // 5 min TTL
            completed: false,
        },
    );
...
/// GET /api/providers/openai-codex/oauth/callback?code=...&state=...
///
/// OAuth redirect callback. The OpenAI authorization server redirects here
/// after the user authorizes. Receives the authorization code and state,
/// matches it to a pending flow, exchanges the code for tokens.
pub async fn codex_oauth_callback(
    ...
    // Find the matching flow by state parameter
    let mut matched_poll_id = None;
    let mut verifier = None;

    for entry in CODEX_FLOWS.iter() {
        if entry.value().state == callback_state && !entry.value().completed {
            matched_poll_id = Some(entry.key().clone());
            verifier = Some(entry.value().verifier.clone());
            break;
        }
    }

    let (poll_id, code_verifier) = match (matched_poll_id, verifier) {
        (Some(pid), Some(v)) => (pid, v),
        _ => {
            return axum::response::Html(
                "<html><body><h2>Authorization failed</h2>\
                 <p>Unknown or expired OAuth state. Please start a new flow.</p></body></html>"
                    .to_string(),
            );
        }
    };
    ...
    match openfang_runtime::codex_oauth::exchange_code(&code, &code_verifier).await {
        Ok(tokens) => {
            // Save tokens to disk
            if let Err(e) = openfang_runtime::codex_oauth::save_tokens(&tokens) {
                tracing::warn!("Failed to save Codex OAuth tokens: {e}");
            }
            ...
            // Refresh auth detection in model catalog
            state
                .kernel
                .model_catalog
                .write()
            ...
            axum::response::Html(
                "<html><body><h2>Authorization successful!</h2>\
                 <p>OpenAI Codex (OAuth) has been connected. You can close this window \
                 and return to OpenFang.</p>\
...
///
/// Poll the status of a Codex OAuth PKCE flow.
/// Returns `pending` (user hasn't completed browser auth yet) or `complete`.
pub async fn codex_oauth_poll(Path(poll_id): Path<String>) -> impl IntoResponse {
    match CODEX_FLOWS.get(&poll_id) {
        Some(flow) => {
            if flow.expires_at <= Instant::now() {
                drop(flow);
            ...
            (
                StatusCode::OK,
                Json(serde_json::json!({"status": "pending", "auth_url": flow.auth_url})),
            )
        }
        None => (
            StatusCode::NOT_FOUND,
...
/// GET /api/providers/openai-codex/oauth/status
///
/// Check if Codex OAuth is currently authenticated and tokens are valid.
pub async fn codex_oauth_status() -> impl IntoResponse {
    let has_tokens = openfang_runtime::codex_oauth::codex_oauth_available();

    if has_tokens {
        if let Some(tokens) = openfang_runtime::codex_oauth::load_tokens() {
        ...
        if let Some(cli_tokens) = openfang_runtime::codex_oauth::load_codex_cli_tokens() {
            return (
                StatusCode::OK,
                Json(serde_json::json!({
                    "authenticated": true,
                    "valid": cli_tokens.is_valid(),
                    "source": "codex-cli",
                    "has_refresh_token": cli_tokens.refresh_token.is_some(),
                    "account_id": cli_tokens.account_id,
...
/// POST /api/providers/openai-codex/oauth/logout
///
/// Delete stored Codex OAuth tokens (log out).
pub async fn codex_oauth_logout(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    match openfang_runtime::codex_oauth::delete_tokens() {
        Ok(()) => {
            // Refresh auth detection
            state
                .kernel
