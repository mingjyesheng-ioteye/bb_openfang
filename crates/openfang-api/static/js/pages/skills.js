...
function skillsPage() {
  return {
    ...
    async installFromPath() {
      ...
      if (!path) {
        this.localError = 'Please enter a directory path.';
        return;
      }
      this.localInstalling = true;
      this.localResult = null;
      this.localError = '';
      try {
        var data = await OpenFangAPI.post('/api/skills/install-from-path', { path: path });
        this.localResult = data;
        OpenFangToast.success('Skill "' + data.name + '" v' + data.version + ' installed (' + data.tools_count + ' tools)');
        this.localPath = '';
        await this.loadSkills();
      } catch(e) {
        this.localError = e.message || 'Install failed';
      }
      this.localInstalling = false;
    },
