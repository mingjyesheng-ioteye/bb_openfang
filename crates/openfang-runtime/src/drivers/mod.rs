...
fn provider_defaults(provider: &str) -> Option<ProviderDefaults> {
    match provider {
        ...
        "codex" | "openai-codex" => Some(ProviderDefaults {
            ...
            api_key_env: "OPENAI_API_KEY",
            key_required: false, // openai-codex uses OAuth, codex falls back to API key
        }),
...
pub fn create_driver(config: &DriverConfig) -> Result<Arc<dyn LlmDriver>, LlmError> {
    ...
    // OpenAI Codex OAuth — PKCE-authenticated driver for the Codex Responses API.
    // Uses browser-based OAuth flow, stores and auto-refreshes tokens.
    if provider == "openai-codex" {
        return Ok(Arc::new(openai_codex::CodexOAuthDriver::new()));
    }
    ...
    Err(LlmError::Api {
        ...
        message: format!(
            "Unknown provider '{}'. Supported: anthropic, gemini, openai, azure, groq, openrouter, \
             ...
             openai-codex, chutes, venice, nvidia, codex, claude-code. \
